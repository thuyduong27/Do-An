...
class SuaNKTT extends StatefulWidget {
  NhatKyTT data;
  Function callbackChange;
  SuaNKTT({Key? key, required this.callbackChange, required this.data}) : super(key: key);

  @override
  _QuanLyDNScreenState createState() => _QuanLyDNScreenState();
}

class _QuanLyDNScreenState extends State<SuaNKTT> {
  ...
  void initState() {
    super.initState();
    data = widget.data;
    time1 = DateFormat('dd-MM-yyyy').format(DateTime.parse(data.startDate!));
    time2 = DateFormat('dd-MM-yyyy').format(DateTime.parse(data.endDate!));
    ten.text = data.name ?? "";
    content.text = data.content ?? "";
    ketQua.text = data.ketQua ?? "";
          ...
          child: ListView(
            ...
            children: [
              TitlePage(
                ...
                content: 'Cập nhật nhật ký thực tập',
              ),
              Container(
                                    ...
                                    child: TextButton(
                                      ...
                                      onPressed: () async {
                                        ...
                                        if (ten.text == "" || content.text == "" || ketQua.text == "" || time1 == null || time2 == null) {
                                          ...
                                          await httpPut("/api/nhat-ky-thuc-tap/put/${data.id}", data.toJson(), context);
                                          showToast(
                                            context: context,
