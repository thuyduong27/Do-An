...
class SuaKHTT extends StatefulWidget {
  KeHoachThucTap keHoachThucTap;
  Function callbackChange;
  SuaKHTT({Key? key, required this.callbackChange, required this.keHoachThucTap}) : super(key: key);

  @override
  _QuanLyDNScreenState createState() => _QuanLyDNScreenState();
}

class _QuanLyDNScreenState extends State<SuaKHTT> {
  ...
  void initState() {
    super.initState();
    data = widget.keHoachThucTap;
    tieuDe.text = widget.keHoachThucTap.tilte ?? "";
    noiDung.text = widget.keHoachThucTap.content ?? "";
    time1 = DateFormat('dd-MM-yyyy').format(DateTime.parse(widget.keHoachThucTap.startDate!));
    time2 = DateFormat('dd-MM-yyyy').format(DateTime.parse(widget.keHoachThucTap.endDate!));
    if (widget.keHoachThucTap.hanDangKy != null) {
      time3 = DateFormat('dd-MM-yyyy').format(DateTime.parse(widget.keHoachThucTap.hanDangKy!));
    }
    if (widget.keHoachThucTap.hanNopBaoCao != null) {
      time4 = DateFormat('dd-MM-yyyy').format(DateTime.parse(widget.keHoachThucTap.hanNopBaoCao!));
    }
    if (widget.keHoachThucTap.ngayDiThuctap != null) {
      time5 = DateFormat('dd-MM-yyyy').format(DateTime.parse(widget.keHoachThucTap.ngayDiThuctap!));
    }
    if (widget.keHoachThucTap.ngayHetThuctap != null) {
      time6 = DateFormat('dd-MM-yyyy').format(DateTime.parse(widget.keHoachThucTap.ngayHetThuctap!));
    }
    selectedTrangThai = widget.keHoachThucTap.status ?? 0;
  }
          ...
          child: ListView(
            ...
            children: [
              TitlePage(
                ...
                content: 'Cập nhật kế hoạch thực tập',
              ),
              Container(
                                    ...
                                    child: Container(
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          Expanded(
                                            flex: 2,
                                            child: Text('File danh sách:', style: titleContainerBox),
                                          ),
                                          Expanded(
                                            flex: 5,
                                            child: Container(
                                    ...
                                    child: TextButton(
                                      ...
                                      onPressed: () async {
                                        ...
                                        if (tieuDe.text == "" || time1 == null || time2 == null || selectedTrangThai == -1) {
                                          ...
                                          if (selectedTrangThai == 0 && data.status != 0) {
                                            var responseCheckStatus = await httpGet("/api/ke-hoach-thuc-tap/get/page?filter=status:0", context);
                                            var resultCheck = jsonDecode(responseCheckStatus["body"]);
                                            var totalElements = resultCheck['result']['totalElements'];
                                            if (totalElements == 0) {
                                              ...
                                              var abc = await httpPut("/api/ke-hoach-thuc-tap/put/${data.id}", data.toJson(), context);
                                              print(data.toJson());
                                              showToast(
                                                context: context,
                                                msg: "Sửa thành công",
                                              ...
                                              showToast(
                                                context: context,
                                                msg: "Đã có kế hoạch thực tập đang thực hiện",
                                                color: Color.fromRGBO(245, 117, 29, 1),
                                                icon: const Icon(Icons.info),
                                              );
                                              Navigator.pop(context);
