...
class SuaDoanhNghiep extends StatefulWidget {
  Company company;
  Function callbackChange;
  SuaDoanhNghiep({Key? key, required this.company,required this.callbackChange}) : super(key: key);

  @override
  _QuanLyDNScreenState createState() => _QuanLyDNScreenState();
}

class _QuanLyDNScreenState extends State<SuaDoanhNghiep> {
  ...
  void initState() {
    super.initState();
    ten.text = widget.company.name ?? "";
    mst.text = widget.company.mst ?? "";
    address.text = widget.company.address ?? "";
    manager.text = widget.company.manager ?? "";
    managerSdt.text = widget.company.managerSdt ?? "";
    managerEmail.text = widget.company.managerEmail ?? "";
    slNhan.text = (widget.company.soLuongNhan != null) ? widget.company.soLuongNhan.toString() : "";
    if (widget.company.hopDong == true) {
      selectedHopDong = 1;
    } else {
      selectedHopDong = 2;
    }
    selectedTrangThai = widget.company.status ?? 0;
    fileHopDong = widget.company.fileHopDong;
  }
                                            ...
                                            child: Row(
                                              ...
                                              children: [
                                                ...
                                                Expanded(
                                                  ...
                                                  child: Container(
                                                      child: TextButton(
                                                    child: Text('${(widget.company.fileHopDong != null) ? widget.company.fileHopDong : "Tải"}'),
                                                    onPressed: () async {
                                                      ...
                                                      if (result != null) {
                                                        ...
                                                        setState(() {
                                                          widget.company.fileHopDong = fileHopDong;
                                                        });
                                                      } else {
                                                        return showToast(
                                                          context: context,
                                    ...
                                    child: TextButton(
                                      ...
                                      onPressed: () async {
                                        ...
                                        if (ten.text == "" || address.text == "" || manager.text == "" || managerSdt.text == "" || managerEmail.text == "" || mst.text == "" || selectedHopDong == 0 || selectedTrangThai == 0) {
                                          ...
                                          widget.company.name = ten.text;
                                          widget.company.address = address.text;
                                          widget.company.manager = manager.text;
                                          widget.company.managerSdt = managerSdt.text;
                                          widget.company.managerEmail = managerEmail.text;
                                          widget.company.mst = mst.text;
                                          widget.company.type = 1;
                                          if (selectedHopDong == 1) {
                                            widget.company.hopDong = true;
                                            widget.company.soLuongNhan = int.tryParse(slNhan.text);
                                          } else {
                                            widget.company.hopDong = false;
                                            widget.company.fileHopDong = null;
                                            widget.company.soLuongNhan = null;
                                          }
                                          if (selectedTrangThai == 1) {
                                            widget.company.status = 1;
                                          } else {
                                            widget.company.status = 2;
                                          }
                                          await httpPut("/api/doanh-nghiep/put/${widget.company.id}", widget.company.toJson(), context);
                                          showToast(
                                            context: context,
                                          ...
                                          widget.callbackChange(widget.company);
                                          Navigator.pop(context);
                                          Navigator.pop(context);
                                        }
                                      },
