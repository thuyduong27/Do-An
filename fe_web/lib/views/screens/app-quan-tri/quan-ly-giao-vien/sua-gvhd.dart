...
class SuaGVHD extends StatefulWidget {
  GVHD gvhd;
  Function callbackChange;
  SuaGVHD({Key? key, required this.callbackChange, required this.gvhd}) : super(key: key);

  @override
  _QuanLyDNScreenState createState() => _QuanLyDNScreenState();
}

class _QuanLyDNScreenState extends State<SuaGVHD> {
  ...
  void initState() {
    super.initState();
    data = widget.gvhd;
    ten.text = widget.gvhd.fullName ?? "";
    donVi.text = widget.gvhd.donVi ?? "";
    sdt.text = widget.gvhd.sdt ?? "";
    email.text = widget.gvhd.email ?? "";
    address.text = widget.gvhd.address ?? "";
