...
class PhanCongGVHDScreen extends StatefulWidget {
  PhanCongGVHDScreen({Key? key}) : super(key: key);

  @override
  _QuanLyDNScreenState createState() => _QuanLyDNScreenState();
}

class _QuanLyDNScreenState extends State<PhanCongGVHDScreen> {
  var page = 1;
  var rowPerPage = 10;
  var totalElements = 0;
  ...
  Future<List<SinhVien>> getData(int page, String findGVHD) async {
    ...
    var response5;
    if (findGVHD == "") {
      response5 = await httpGet("/api/sinh-vien-thuc-tap/get/page?size=$rowPerPage&page=$page&filter=idKhtt:${selectedKHTT.id ?? 0} and status:2 and idGvhd is null", context);
    } else {
      response5 = await httpGet("/api/sinh-vien-thuc-tap/get/page?size=$rowPerPage&page=$page&filter=idKhtt:${selectedKHTT.id ?? 0}and status:2 and idGvhd is null and $findGVHD", context);
    }
                                                ...
                                                Container(
                                                  ...
                                                  child: TextButton(
                                                    style: TextButton.styleFrom(
                                                      ...
                                                      backgroundColor: (listSVChon.isNotEmpty) ? mainColor : grey,
                                                      textStyle: Theme.of(context).textTheme.bodySmall?.copyWith(fontSize: 10.0, letterSpacing: 2.0),
                                                    ),
                                                    onPressed: (listSVChon.isNotEmpty)
                                                        ? () async {
                                                            // for (SinhVien element in listSVChon) {
                                                            //   print(element.nguoiDung!.fullName);
                                                            // }
                                                    ...
                                                    child: Row(
                                                      children: [
                                                        ...
                                                        SizedBox(width: 5),
                                                        Text('Chọn GVHD', style: textBtnWhite),
                                                      ],
                                                    ),
                                                  ),
                                                ),
                                                  ...
                                                  child: TextButton(
                                                    ...
                                                    onPressed: () {
                                                      ...
                                                      if (selectedCSTT.id != null) {
                                                        findDoanhNghiep = "and idDoanhNghiep:${selectedCSTT.id} ";
                                                      } else
                                                        findDoanhNghiep = "";
                                                      findGVHD = findTen + findNganh + findDoanhNghiep;
                                            ...
                                            onSelectChanged: (bool? selected) {
                                              setState(
                                                () {
                                                  ...
                                                  if (_selectedDataRow[i] == true) {
                                                    listSVChon.add(listData[i]);
                                                  } else {
                                                    listSVChon.remove(listData[i]);
                                                  }
                                                },
                                              );
                                            },
...
class XacNhanThem extends StatefulWidget {
  ...
  int idKhtt;
  Function callBack;
  XacNhanThem({Key? key, required this.listSinhVien, required this.idKhtt, required this.callBack}) : super(key: key);
  @override
      ...
      setState(() {
        ...
        if (content.isNotEmpty) {
          for (var i = 0; i < content.length; i++) {
            ...
            resultGVHD[i].daNhan = content[i]['soLuong'];
            print(content[i]['soLuong']);
          }
        }
      });
      GVHD all = GVHD(
        id: null,
        fullName: "--Chọn giảng viên--",
      );
      setState(() {
        ...
        child: ListView(
          children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('Phân công GVHD', style: textTitleAlertDialog),
              IconButton(
                  onPressed: () {
                  ...
                  Expanded(
                    ...
                    child: Container(
                      ...
                      child: DropdownSearch<GVHD>(
                        ...
                        itemAsString: (GVHD u) => "${(u.hocVi == 1) ? "Ths" : (u.hocVi == 2) ? "Tiến sĩ" : ""} ${u.fullName ?? ""}  ${(u.gioiHanNhan != null) ? "(${u.daNhan}/${u.gioiHanNhan})" : ""}",
                  ...
                  child: TextButton(
                    ...
                    onPressed: () async {
                      ...
                      if (selectedGVHD.id == null) {
                        showToast(
                          context: context,
                          msg: "Chọn giáo viên hướng dẫn",
                          color: Colors.orange,
                          icon: const Icon(Icons.warning),
                        );
                        Navigator.pop(context);
                      } else {
                        if ((selectedGVHD.gioiHanNhan! - selectedGVHD.daNhan!) < widget.listSinhVien.length) {
                          showToast(
                            context: context,
                            msg: "Số lượng sinh viên lớn",
                            color: Colors.orange,
                            icon: const Icon(Icons.warning),
                          );
                          Navigator.pop(context);
                        } else {
                          for (SinhVien element in widget.listSinhVien) {
                            element.idGvhd = selectedGVHD.id;
                            await httpPut("/api/sinh-vien-thuc-tap/put/${element.id}", element.toJson(), context);
                          }
                          showToast(
                            context: context,
                            msg: "Phân công GVHD thành công",
