...
class ThemMoiSinhVien extends StatefulWidget {
  int idKhtt;
  Function callbackChange;
  ThemMoiSinhVien({Key? key, required this.idKhtt, required this.callbackChange}) : super(key: key);

  @override
  _QuanLyDNScreenState createState() => _QuanLyDNScreenState();
}

class _QuanLyDNScreenState extends State<ThemMoiSinhVien> {
                                    ...
                                    child: TextButton(
                                      ...
                                      onPressed: () async {
                                        ...
                                        if (ten.text == "" || email.text == "" || sdt.text == "" || selectedTrangThai == -1 || selectedNganh == -1 || ngaySinh == null || lop.text == "" || khoa.text == "" || masv.text == "") {
                                          ...
                                          var abc = await httpPost("/api/nguoi-dung/create", data.nguoiDung!.toJson(), context);
                                          var abcRf = jsonDecode(abc["body"]);
                                          var idNguoidung = abcRf['result'];
                                          data.idNguoiDung = int.tryParse(idNguoidung.toString());
                                          print(data.toJson());
                                          await httpPost("/api/sinh-vien-thuc-tap/post", data.toJson(), context);
                                          showToast(
                                            context: context,
